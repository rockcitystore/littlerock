...
class _C extends Taro.Component {
  ...
  render() {
    ...
    return (
      <View
        className={
          extClass + ' weui-cells__group ' + outerClass + ' ' + childClass
        }
      >
        {title && <View className="weui-cells__title">{title}</View>}
        <View
          className={
            'weui-cells weui-cells_after-title weui-cells_form ' +
            (checkboxCount > 0 && checkboxIsMulti ? 'weui-cells_checkbox' : '')
          }
        >
          {this.props.children}
        </View>
        {footer ? (
          <View className="weui-cells__tips">{footer}</View>
        ) : (
