  ...
  /******/ [
    ...
    /***/ function(module, exports, __webpack_require__) {
      ...
      var FormValidator = (function() {
          ...
          return new Promise(function(resolve) {
            ...
            var _loop = function _loop(k) {
              ;(function(fieldName) {
                ...
                _this._innerValidateField(k, models[fieldName], function(
                  valid,
                  newError
                ) {
                  fieldCount++
        ...
        FormValidator.prototype.validateField = function validateField(
          name,
          value
        ) {
          var _this2 = this
        ...
        FormValidator.prototype._innerValidateField = function _innerValidateField(
          name,
          value
        ) {
          var cb =
          ...
          if (toString.call(rules) === '[object Array]') {
            rules.forEach(function(rule) {
              rule.name = name
              var resMessage = validateSingleRule(
                rule,
                value || models[name],
                rule.param,
                models
        ...
        FormValidator.prototype.addMethod = function addMethod(
          ruleName,
          method
        ) {
    ...
    /***/ function(module, exports, __webpack_require__) {
      ...
      exports.default = {
        ...
        min: function min(r, val) {
          var min = r.min
          if (val < min)
            return string_1.sprintf(r.message || defaultMessage.min, min)
        },
        max: function max(r, val) {
          var max = r.max
          if (val > max)
            return string_1.sprintf(r.message || defaultMessage.max, max)
        },
        ...
        mobile: function mobile(r, val) {
          ...
          val = val || ''
          if (val.length !== 11)
            return string_1.sprintf(r.message || defaultMessage.mobile)
        },
        ...
        equalTo: function equalTo(r, value, param, models) {
          ...
          if (value !== models[equalTo])
            return string_1.sprintf(r.message || defaultMessage.equalTo, r.name)
        },
...
@withWeapp({
  ...
  attached: function attached() {
    this.initRules()
    this.formValidator = new form_validator_1.default(
      this.data.models,
      this.data.newRules
    )
  },
  ...
  methods: {
    ...
    _modelChange: function _modelChange(newVal, oldVal, path) {
      ...
      if (diffObj) {
        ;(function() {
          ...
          var _loop = function _loop(k) {
            _this.formValidator.validateField(k, diffObj[k], function(
              isValided,
              error
            ) {
              if (error && error[k]) {
    ...
    validate: function validate(cb) {
      ...
      return this.formValidator.validate(function(isValid, errors) {
        ...
        _this2.triggerEvent(
          ...
          isValid
            ? { trigger: 'validate' }
            : { errors: handleError, trigger: 'validate' }
        )
    ...
    validateField: function validateField(name, value) {
      ...
      return this.formValidator.validateField(name, value, function(
        isValid,
        errors
      ) {
...
class _C extends Taro.Component {
  ...
  render() {
    const { extClass } = this.data
    return <View className={extClass}>{this.props.children}</View>
  }
}
